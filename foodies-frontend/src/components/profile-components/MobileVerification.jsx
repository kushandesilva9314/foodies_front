...
const MobileVerification = ({ mobile, isVerified, onVerified }) => {
  ...
  // OTP input handlers (same pattern as Signup)
  ...
  // "Verify Now" / "Resend Code" — sends OTP to the mobile number
  const handleSendOtp = async () => {
    setSendLoading(true);
    setError("");
    try {
      // TODO: trigger Firebase phone-auth → send OTP to `mobile`
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setError("");
    try {
      // TODO: verify OTP via Firebase, then call backend to set is_mobile_verified = true
      await new Promise((r) => setTimeout(r, 800));
    ...
    >
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-base font-bold text-gray-800">Mobile Verification</h3>
            <p className="text-xs text-gray-500">Verify your mobile number to secure your account</p>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 bg-white/70 border border-gray-100 rounded-2xl px-4 py-3">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-sm font-semibold text-gray-700">{mobile || "No number on file"}</span>
        </div>
        ...
        {!isVerified && !showOtpInput && (
          ...
          >
            {sendLoading ? <><Spinner /><span>Sending...</span></> : <><Send className="w-3.5 h-3.5" /><span>Verify Now</span></>}
          </motion.button>
        )}
      </div>
      ...
      {/* Error */}
      {error && (
        <motion.p initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} className="text-red-600 text-xs ml-1">
          {error}
        </motion.p>
      )}
        ...
        {!isVerified && showOtpInput && (
          ...
          >
            ...
            <div className="text-center">
              {resendTimer > 0 ? (
                <p className="text-xs text-gray-600">
                  Resend code in <span className="font-bold text-orange-600">{resendTimer}s</span>
                </p>
              ) : (
                <button
                  type="button"
            ...
            <div className="flex gap-3">
              ...
              >
                {verifyLoading ? <><Spinner /><span>Verifying...</span></> : <span>Verify Code</span>}
              </motion.button>
            </div>
          </motion.form>
        )}
